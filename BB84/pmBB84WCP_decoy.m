...
function [protocolDescription,channelModel,leakageEC]=setDescription()
    ...
    channelModel=str2func('pmBB84WCPChannel_Choi'); %improved decoy methods
    % channelModel=str2func('pmBB84WCPChannel'); %standard decoy methods
...
function parameters=setParameters()
    ...
    %parameters.names = ["ed","pz","pd","eta","etad","f","mu1","mu2","active","fullstat"]; %BB84 Decoy
    ...
    parameters.scan.eta = 10.^(-linspace(0,3.5,20));% channel transmittance

    %%%%%%%%%%%%%%%% 2.fixed parameters %%%%%%%%%%%%%%%%
    %optional; the constant values can be either numerical or array/matrices
    
    parameters.fixed.ed = 0; %0.01; %misalignment, defined as single-photon error, i.e. sin^2(theta)
    ...
    parameters.optimize.mu1 = [0.1,0.9,1.5]; %can optionally optimize laser intensity (need to comment it out mu1 in 'fixed' category)
