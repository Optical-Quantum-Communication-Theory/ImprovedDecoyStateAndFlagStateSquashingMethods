...
function [protocolDescription,channelModel,leakageEC,parameters,solverOptions] = pmBB84WCP_nodecoy()
...
function [protocolDescription,channelModel,leakageEC]=setDescription()
    ...
    % channelModel=str2func('pmBB84WCPChannel_constr_nodecoy'); %contains asymptotic decoy state analysis in file
    channelModel=str2func('pmBB84WCPChannel_Choi_nodecoy');
...
function parameters=setParameters()
    ...
    parameters.names = ["ed","pz","pd","eta","etad","f","mu1","active","fullstat"]; %BB84 Decoy
    ...
    eta_list = 10.^(-linspace(0,3.5,20));
    parameters.scan.eta = eta_list(1:9);% channel transmittance
