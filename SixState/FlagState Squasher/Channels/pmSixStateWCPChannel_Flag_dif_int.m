...
function channelModel = pmSixStateWCPChannel_Flag_dif_int(protocolDescription,names,p)
    ...
    %Probability of signal was sent, i.e. p(x)
    probList_signal = [pzA/2; pzA/2; pxA/2; pxA/2; pyA/2; pyA/2];
    
    %Probability of sending a single photon
    P1 = Poisson(signal_int,1)*probList_signal;

    %Z basis
    pH = Poisson(signal_int(1),1)*1/2*pzA/P1;
    pV = Poisson(signal_int(2),1)*1/2*pzA/P1;
    
    %X basis
    pD = Poisson(signal_int(3),1)*1/2*pxA/P1;
    pA = Poisson(signal_int(4),1)*1/2*pxA/P1;

    %Y basis
    pR = Poisson(signal_int(5),1)*1/2*pyA/P1;
    pL = Poisson(signal_int(6),1)*1/2*pyA/P1;
    
    probList_single_photon = [pH; pV; pD; pA; pR; pL];
