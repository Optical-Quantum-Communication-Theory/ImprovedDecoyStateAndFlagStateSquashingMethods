...
function [protocolDescription,channelModel,leakageEC,parameters,solverOptions] = pmSixStateWCP_decoy_Flag()
...
function parameters=setParameters()
    ...
    parameters.scan.eta = 10.^(-linspace(0,3.5,12)); %channel transmittance
    % parameters.scan.eta = 10.^(-linspace(0,1.2,8)); %channel transmittance for single intensity
    ...
    %biased basis choice 
    parameters.fixed.pzA = 0.8; %basis choice probability for Alice's Z basis
    parameters.fixed.pxA = 0.1; %basis choice probability for Alice's X basis
    parameters.fixed.pyA = 0.1; %basis choice probability for Alice's Y basis
    parameters.fixed.pzB = 0.8; %basis choice probability for Bob's Z basis
    parameters.fixed.pxB = 0.1; %basis choice probability for Bob's X basis
    parameters.fixed.pyB = 0.1; %basis choice probability for Bob's Y basis
    
    % %unbiased basis choice
