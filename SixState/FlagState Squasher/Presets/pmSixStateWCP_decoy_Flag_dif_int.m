...
function [protocolDescription,channelModel,leakageEC,parameters,solverOptions] = pmSixStateWCP_decoy_Flag_dif_int()
...
function [protocolDescription,channelModel,leakageEC]=setDescription()
    ...
    protocolDescription=str2func('pmSixStateLossyDescription_Flag_dif_int');%different intensities w/o bit bias
    channelModel=str2func('pmSixStateWCPChannel_Flag_dif_int'); %different intensities w/o bit bias


    % protocolDescription=str2func('pmSixStateLossyDescription_Flag_bit_bias'); %different intensities with bit bias
    % channelModel=str2func('pmSixStateWCPChannel_Flag_bit_bias'); %different intensities with bit bias
...
function parameters=setParameters()
    ...
    parameters.names = ["ed","pzA","pxA","pyA","pzB","pxB","pyB","pd","eta","etad","f","mu1","mu2","mu3","fullstat"]; %SixState Decoy
    %parameters.names = ["ed","pzA","pxA","pyA","pzB","pxB","pyB","pd","eta","etad","f","mu1","mu2","fullstat"]; %SixState Decoy
    %parameters.names = ["ed","pzA","pxA","pyA","pzB","pxB","pyB","pd","eta","etad","f","mu1","fullstat"]; %SixState Decoy
    ...
    parameters.scan.eta = 10.^(-linspace(1e-3,3,10)); %channel transmittance 
    ...
    parameters.fixed.pzA = 1/3; %basis choice probability for Alice's Z basis
    parameters.fixed.pxA = 1/3; %basis choice probability for Alice's X basis
    parameters.fixed.pyA = 1/3; %basis choice probability for Alice's Y basis
    parameters.fixed.pzB = 1/3; %basis choice probability for Bob's Z basis
    parameters.fixed.pxB = 1/3; %basis choice probability for Bob's X basis
    parameters.fixed.pyB = 1/3; %basis choice probability for Bob's Y basis
    ...
    parameters.fixed.mu1 = [0.95,0.15,0.1,0.9,0.95,0.1];%first intensity (signal)
    parameters.fixed.mu2 = ones(1,6)*0.01; %second intensity (decoy 1)
    parameters.fixed.mu3 = ones(1,6)*0.001; %third intensity (decoy 2)
...
function solverOptions=setOptions()
    ...
    solverOptions.solver1.maxiter = 20;
    ...
    solverOptions.solver1.cvxSolver = 'mosek';
    solverOptions.solver1.cvxPrecision = 'high';
