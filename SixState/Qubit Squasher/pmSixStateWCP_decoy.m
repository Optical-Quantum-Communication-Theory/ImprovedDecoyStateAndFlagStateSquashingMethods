...
function [protocolDescription,channelModel,leakageEC,parameters,solverOptions] = pmSixStateWCP_decoy()
...
function [protocolDescription,channelModel,leakageEC]=setDescription()
    ...
    protocolDescription=str2func('pmSixStateLossyDescription');
    channelModel=str2func('pmSixStateWCPChannel'); %contains asymptotic decoy state analysis in file
    % channelModel=str2func('pmSixStateWCPChannel_improved_decoy'); %improved decoy methods
...
function parameters=setParameters()
    ...
    parameters.names = ["ed","pzA","pxA","pzB","pxB","pd","eta","etad","f","mu1","mu2","mu3","fullstat"]; %SixState Decoy
    %parameters.names = ["ed","pzA","pxA","pzB","pxB","pd","eta","etad","f","mu1","mu2","fullstat"]; %SixState Decoy
    ...
    parameters.scan.eta = 10.^(-linspace(1e-3,3.5,12)); %channel transmittance
    ...
    % parameters.fixed.mu1 = 0.55; %first intensity (signal)
    parameters.fixed.mu2 = 0.1; %second intensity (decoy 1)
    parameters.fixed.mu3 = 0.001; %third intensity (decoy 2)
    parameters.fixed.active = 0; %0/1 represents using passive/active detection
...
function solverOptions=setOptions()
    ...
    solverOptions.globalSetting.verboseLevel = 3; 
